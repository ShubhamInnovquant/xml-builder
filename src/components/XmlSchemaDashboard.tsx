...
import { FileCode, Plus, FileDown, FileUp, Trash2, Edit, Download } from 'lucide-react';
import { downloadJSON, readFileAsText } from '../utils/helpers';

const XmlSchemaDashboard = () => {
  const { schemas, createSchema, selectSchema, deleteSchema, exportSchema, importSchema } = useXmlSchemaStore();
  ...
  const handleExportAll = () => {
    ...
    downloadJSON(JSON.stringify(allSchemas, null, 2), `all-xml-schemas-${Date.now()}.json`);
  };
        ...
        <div style={{ maxWidth: '1280px', margin: '0 auto', position: 'relative', zIndex: 10 }}>
          <div className="text-center" style={{ marginBottom: '64px' }}>
            <div className="inline-flex items-center justify-center" style={{ padding: '16px', background: 'linear-gradient(to bottom right, rgba(6, 182, 212, 0.2), rgba(20, 184, 166, 0.2))', borderRadius: '24px', marginBottom: '32px', backdropFilter: 'blur(12px)', border: '1px solid rgba(6, 182, 212, 0.3)' }}>
              <FileCode size={48} className="text-electric-400" />
            </div>
            ...
            <p className="text-gray-300" style={{ fontSize: '20px', maxWidth: '672px', margin: '0 auto 40px', lineHeight: '1.6' }}>
              Create and manage complex XML schemas with elements, attributes, and text content. Build hierarchical structures visually.
            </p>
          ...
          <div className="grid" style={{ gridTemplateColumns: 'repeat(3, 1fr)', gap: '24px', marginBottom: '64px' }}>
            <div className="card-hover text-center">
              <div className="w-16 h-16 mx-auto mb-4 bg-gradient-to-br from-electric-500/20 to-teal-500/20 rounded-2xl flex items-center justify-center border border-electric-500/30">
                <FileCode size={32} className="text-electric-400" />
              </div>
              <h3 className="text-xl font-semibold mb-2 text-gray-100">Hierarchical Structure</h3>
              <p className="text-gray-400">Define nested elements and attributes for complex XML</p>
            </div>
            <div className="card-hover text-center">
              <div className="w-16 h-16 mx-auto mb-4 bg-gradient-to-br from-electric-500/20 to-teal-500/20 rounded-2xl flex items-center justify-center border border-electric-500/30">
                <FileCode size={32} className="text-electric-400" />
              </div>
              <h3 className="text-xl font-semibold mb-2 text-gray-100">Live XML Preview</h3>
              <p className="text-gray-400">Instantly see your XML structure as you build it</p>
            </div>
            <div className="card-hover text-center">
              <div className="w-16 h-16 mx-auto mb-4 bg-gradient-to-br from-electric-500/20 to-teal-500/20 rounded-2xl flex items-center justify-center border border-electric-500/30">
                <FileCode size={32} className="text-electric-400" />
              ...
              <p className="text-gray-400">Save and load your XML schemas as JSON files</p>
            </div>
          </div>
        </div>
      </section>
        ...
        <div style={{ maxWidth: '1280px', margin: '0 auto' }}>
          {schemas.length === 0 ? (
            <div className="card text-center" style={{ padding: '80px 24px' }}>
              ...
              <p className="text-gray-400" style={{ fontSize: '18px', marginBottom: '32px' }}>Create your first XML schema to get started</p>
              <button
                onClick={() => setShowCreateModal(true)}
                className="btn-primary inline-flex items-center gap-2"
              >
            ...
            <>
              <div style={{ marginBottom: '32px' }}>
                ...
                <p className="text-gray-400" style={{ fontSize: '16px' }}>Manage and edit your XML schema collection</p>
              </div>
                      ...
                      <div className="flex-1">
                        ...
                        <div className="flex items-center" style={{ gap: '12px', fontSize: '14px' }}>
                          <span className="rounded-lg bg-white/5 backdrop-blur-sm border border-white/10 text-gray-300" style={{ padding: '6px 12px' }}>
                            {schema.elements.length} root elements
                          </span>
                          <span className="rounded-lg bg-white/5 backdrop-blur-sm border border-white/10 text-gray-300" style={{ padding: '6px 12px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
              <div>
                ...
                <input
                  ...
                  placeholder="e.g., Product Catalog Schema"
...
export default XmlSchemaDashboard;
